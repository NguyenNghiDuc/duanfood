...
const foodToCategory = (title, description) => {
  ...
  const drinkKeywords = ['trà', 'nước', 'juice', 'mứt', 'milk', 'latte', 'cà phê', 'coffee', 'sữa', 'matcha', 'trà sữa', 'soda', 'nước ép', 'smoothie']
  const dessertKeywords = ['tráng miệng', 'chè', 'bánh', 'dessert', 'kem', 'pudding']

  for (const k of dessertKeywords) if (s.includes(k)) return 'Tráng miệng'
  for (const k of drinkKeywords) if (s.includes(k)) return 'Đồ uống'
  return 'Đồ ăn'
}
...
async function classify() {
  try {
    ...
    if (!mapping['Đồ ăn'] || !mapping['Đồ uống'] || !mapping['Tráng miệng']) {
      console.log('Categories missing, creating defaults...')
      await db.query("INSERT OR IGNORE INTO categories (name) VALUES ('Đồ ăn'), ('Đồ uống'), ('Tráng miệng')")
